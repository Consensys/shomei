...
public class LineaGetTrielogProofTest {
  ...
  public void shouldReturnCorrectMethodName() {
    final LineaGetTrielogProof method =
        new LineaGetTrielogProof(worldStateArchive, trieLogLayerConverter);
  ...
  public void shouldReturnErrorWhenParentBlockMissing() {
    ...
    final JsonRpcRequestContext request =
        createRequest(serializedTrieLog, testParentBlockHash.toHexString());
  ...
  public void shouldReturnErrorWhenTrielogSerializationInvalid() {
    ...
    final JsonRpcRequestContext request =
        createRequest(invalidSerializedTrieLog, testParentBlockHash.toHexString());
  ...
  public void shouldReturnProofsForValidTrielogWithAccountAndStorageChanges() {
    ...
    final ZkEvmWorldState realWorldState =
        createRealWorldState(Map.of(testPriorAccount, Map.of(storageSlot, UInt256.valueOf(10))));
    ...
    testTrieLogLayer.addStorageChange(
        testAccountKey,
        new StorageSlotKey(UInt256.valueOf(1)),
        UInt256.valueOf(10),
        UInt256.valueOf(42),
        false);
  ...
  public void shouldReturnProofsForTrielogWithOnlyAccountChanges() {
    ...
    final ZkEvmWorldState worldStateForAccountOnly =
        createRealWorldState(Map.of(priorAccountNoStorage, Map.of()));
    ...
    // Setup method with real world state
    final LineaGetTrielogProof methodForAccountOnly =
        new LineaGetTrielogProof(worldStateArchive, trieLogLayerConverter);
    lenient()
        .when(worldStateArchive.getCachedWorldState(eq(testParentBlockHash)))
  ...
  public void shouldReturnEmptyProofsForEmptyTrielog() {
    ...
    final LineaGetTrielogProof methodForEmptyTest =
        new LineaGetTrielogProof(worldStateArchive, trieLogLayerConverter);
    lenient()
        .when(worldStateArchive.getCachedWorldState(eq(testParentBlockHash)))
  ...
  public void shouldHandleMultipleAccountsAndStorageSlots() {
    ...
    final ZkEvmWorldState multiAccountWorldState =
        createRealWorldState(
            Map.of(
                ...
                priorAccount2,
                    Map.of(new StorageSlotKey(UInt256.valueOf(1)), UInt256.valueOf(20))));
    ...
    multiAccountTrieLog.addStorageChange(
        testAccountKey,
        new StorageSlotKey(UInt256.valueOf(1)),
        UInt256.valueOf(5),
        UInt256.valueOf(42),
    ...
    final LineaGetTrielogProof methodForMultiTest =
        new LineaGetTrielogProof(worldStateArchive, trieLogLayerConverter);
    lenient()
        .when(worldStateArchive.getCachedWorldState(eq(testParentBlockHash)))
  ...
  private JsonRpcRequestContext createRequest(
      final String serializedTrieLog, final String parentBlockHash) {
    return new JsonRpcRequestContext(
        new JsonRpcRequest(
            "2.0", "linea_getTrielogProof", new Object[] {serializedTrieLog, parentBlockHash}));
  }

  /**
   * Creates a real ZkEvmWorldState initialized with the specified accounts and their storage. This
   * eliminates the need for complex mocks and provides a reliable foundation for testing.
   */
  private ZkEvmWorldState createRealWorldState(
      Map<ZkAccount, Map<StorageSlotKey, UInt256>> accountsWithStorage) {
    ...
    final ZkEvmWorldState worldState =
    ...
    for (Map.Entry<ZkAccount, Map<StorageSlotKey, UInt256>> entry :
        accountsWithStorage.entrySet()) {
      ...
      for (Map.Entry<StorageSlotKey, UInt256> storageEntry : storage.entrySet()) {
        setupLayer.addStorageChange(
            accountKey, storageEntry.getKey(), null, storageEntry.getValue(), false);
      }
    }
